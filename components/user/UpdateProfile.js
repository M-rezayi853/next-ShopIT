...
const UpdateProfile = () => {
  ...
  const { user: userLoaded, loading } = useSelector((state) => state.loadedUser)
  const {
    isUpdated,
  ...
  useEffect(() => {
    if (userLoaded) {
      ...
      setAvatarPreview(userLoaded.avatar.url)
    }
  ...
  }, [dispatch, error, userLoaded, router, isUpdated, alert])
            ...
            <div className='row wrapper'>
              <div className='col-10 col-lg-5'>
                <form className='shadow-lg' onSubmit={submitHandler}>
                  ...
                  >
                    {loadingUpdated ? <ButtonLoader /> : 'Update'}
                  </button>
                </form>
              </div>
            </div>
